...
def create_resources(bucket_name):
    ...
    s3.Bucket(bucket_name).upload_file(input_file_name,
                                       'input/{}'.format(input_file_name+'-'+file_identifier))
...
def convert_speech_to_text(job_name):
    ...
    client.start_transcription_job(
        ...
        Media={
            'MediaFileUri': 's3://{}/input/{}'.format(bucket_name, job_name)},
    ...
    if status['TranscriptionJob']['TranscriptionJobStatus'] == 'COMPLETED':
        ...
        text = ast.literal_eval(obj.get()['Body'].read().decode(
